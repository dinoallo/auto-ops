...
这个 recipe 会生成预置的 Kubernetes root CA 替换材料和预置 ServiceAccount 签名 key pair。它会默认在 play 的第一台控制平面主机上写入 `ca-new-<renewal_id>.crt`、`ca-new-<renewal_id>.key`、`ca-bundle-<renewal_id>.crt`、`sa-new-<renewal_id>.key` 和 `sa-new-<renewal_id>.pub`，然后把生成文件分发到其他目标主机。
...
5. 用当前 `ca.crt` 加 `ca-new-<renewal_id>.crt` 生成 `ca-bundle-<renewal_id>.crt`。
6. 生成 `sa-new-<renewal_id>.key` 并推导 `sa-new-<renewal_id>.pub`。
