...
- `staged_etcd_ca_cert`: 预置 etcd CA 证书，默认 `etcd_pki_dir + '/ca-new.crt'`
- `staged_etcd_ca_key`: 预置 etcd CA 私钥，默认 `etcd_pki_dir + '/ca-new.key'`
...
- `apiserver_etcd_client_cert_output`: 续签后的 apiserver-etcd-client 证书输出路径，默认 `pki_dir + '/apiserver-etcd-client-new.crt'`
- `apiserver_etcd_client_key_output`: 续签后的 apiserver-etcd-client 私钥输出路径，默认 `pki_dir + '/apiserver-etcd-client-new.key'`
- `healthcheck_client_cert_output`: 续签后的 healthcheck client 证书输出路径，默认 `etcd_pki_dir + '/healthcheck-client-new.crt'`
- `healthcheck_client_key_output`: 续签后的 healthcheck client 私钥输出路径，默认 `etcd_pki_dir + '/healthcheck-client-new.key'`
- `etcd_peer_cert_output`: 续签后的 etcd peer 证书输出路径，默认 `etcd_pki_dir + '/peer-new.crt'`
- `etcd_peer_key_output`: 续签后的 etcd peer 私钥输出路径，默认 `etcd_pki_dir + '/peer-new.key'`
- `etcd_server_cert_output`: 续签后的 etcd server 证书输出路径，默认 `etcd_pki_dir + '/server-new.crt'`
- `etcd_server_key_output`: 续签后的 etcd server 私钥输出路径，默认 `etcd_pki_dir + '/server-new.key'`
