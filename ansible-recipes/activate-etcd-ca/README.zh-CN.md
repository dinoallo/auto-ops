...
8. 将 `--etcd-cafile`、`--trusted-ca-file` 和 `--peer-trusted-ca-file` 配置为指向当前生效的 etcd CA 证书。
9. 校验受管理的静态 Pod manifest 参数。
10. 打印当前生效 CA 证书里的证书数量、subject、issuer、有效期和 SHA256 指纹。
